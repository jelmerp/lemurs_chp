...
pops <- read.delim(infile_pops, as.is = TRUE) %>%
  dplyr::filter(sp %in% focal_sp) %>%
  dplyr::select(site, site_short, site_lump, pop2, sp)
cols <- read.delim(infile_cols, as.is = TRUE) %>%
  dplyr::select(popID_short, col)

sites <- merge(sites_raw, pops, by = c('site', 'sp')) %>%
  merge(., cols, by.x = 'pop2', by.y = 'popID_short', all.x = TRUE) %>%
  mutate(pop2 = fct_relevel(as.factor(pop2), c('mmit', 'leh_N', 'leh_S', 'leh_hi'))) %>%
  arrange(pop2) %>%
  mutate(col = fct_inorder(col))

## Population labels:
sites$poplabel <- gsub("mmit", "mittermeieri", sites$pop2)
sites$poplabel <- gsub("leh_N", "lehi: north", sites$poplabel)
sites$poplabel <- gsub("leh_S", "lehi: south", sites$poplabel)
sites$poplabel <- gsub("leh_hi", "lehi: high", sites$poplabel)
...
################################################################################
#### MAP OF ALL OF MADA WITH VEGETATION TYPES ####
################################################################################
...
p_pops <- ggplot() +
  ...
  scale_fill_identity(name = 'Pop. group:',  guide = 'legend',
                      breaks = sites$col, labels = sites$poplabel) +
  new_scale_color() +
...
################################################################################
#### MAP WITH NAMED SAMPLING SITES ####
################################################################################
## Every site separate:
p_sites_each <- ggplot() +
  geom_sf(data = mada_map_rne, fill = 'grey90', color = 'grey30', lwd = 1) +
  geom_point(data = sites, aes(x = lon, y = lat, fill = col),
             color = 'black', shape = 21, size = 6, stroke = 2) +
  scale_fill_identity(breaks = sites$col) +
...
ggsave(paste0(out_map_base, 'sites.png'), p_sites, width = 8, height = 7)
system(paste0('xdg-open ', out_map_base, 'sites.png'))
